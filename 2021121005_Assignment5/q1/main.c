    ...
    while(st[i].done!=1){
        ...
        if(cs[pref_1].curr_slots<probs[pref_1]){
            ...
            if(random<=interest_prob){
                printf(MAGENTA "Student %d has selected the course %s permanently\n" RESET,i,cs[pref_1].course_name);
                st[i].done=1;
                pthread_mutex_lock(&stud_total);
                student_done+=1;
                pthread_mutex_unlock(&stud_total);
                ...
                if(old<3){
                    ...
                    printf(MAGENTA "Student %d could not get any of his preferred courses\n" RESET,i);
                    st[i].done=1;
                    pthread_mutex_lock(&stud_total);
                    student_done+=1;
                    pthread_mutex_unlock(&stud_total);
...
int conduct_tut(int ind){
    while(student_done!=num_students){
        ...
        if(cs[ind].found!=1 && cs[ind].curr_slots!=0){
            pthread_cond_broadcast(&tutsearch[ind]);
            pthread_cond_broadcast(&tutfin[ind]);
            pthread_cond_broadcast(&cond_var[ind]);
            pthread_mutex_lock(&cour_locks[ind]);
            cs[ind].found=-1;
            printf(CYAN "Course %s does not have any TA mentors eligible and is removed from course offerings\n",cs[ind].course_name);
            pthread_mutex_unlock(&cour_locks[ind]);
            break;
        }
        else{
